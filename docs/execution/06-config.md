...
- **Налаштуваннями середовища**: `gateway` (host/IP/port, TLS, CORS, rate-limit), `services.transports (налаштування для транспортів, які реалізує платформа)`, логування, метрики, безпека тощо.  
- **Даними для логіки**: підключення до баз даних, кешів, сторонніх API, feature flags та інші опції для дій вузла.  
...
- **gateway.events** — опис джерел подій. Тут можна вказати транспорт і параметри підключення. Це рекомендація, а не вимога протоколу.  
...
- **domain** — будь-які налаштування для дій вузла.  
...
- Реалізація DNIP може читати **параметри середовища** (`gateway.http`, `gateway.events.sources`, `services.transports (налаштування для транспортів, які реалізує платформа)`, `logger`, `metrics`, `security`).  
